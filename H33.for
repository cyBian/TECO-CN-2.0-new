...
!     Default C/N ratios of Duke FACE
      data CN0 /35.,354.,67.,40.,300.,10.,15.,8./
...
! for diagnosing
 ...
 !Xia:close
      ...
      read(10,11)commts
      open(63,file="annual_duke_tecocnH33.csv")
...
!	  QC=(/100.,4300.,500.,100.,694.,1000.,1585.,4000./) !initial data
!          QC=(/152.,2852.,273.,712.,950.,87.,848.,1634./)
           QC=(/153.,5350.,237.,896.,1715.,400.,1353.,5233./)
...
!                 update vcmx0 and eJmx0 according to C/N of leaves
                  Vcmx0 =Amin1(Vcmax0*SNvcmax,kappaVcmax*SNvcmax)
!                  eJmx0 = 2.7*Vcmx0  ! original
                  eJmx0 = 1.67*Vcmx0  ! Du:O-CN model
...
!                 THE Fourth PART: simulating C influx allocation in pools
     ...
     &               S_T,SNleaf,SNwood,SNroot,!SNfine,SNcoarse,SNmicr,
     &               SNslow,SNpass,LAI,LAIMAX,kappaVcmax,N_immob,N_loss)
...
!Du added CN,QN for test  
     ...
     &	     SNleaf,SNwood,SNroot,SNfine,SNcoarse,SNmicr,SNslow,SNpass
...
162           format(I6,",",f8.0,",",120(f12.4,","))
            ...
            stor_use=Storage/times_storage_use
			NPP_yrc=NPP_yr   !Du: for mineralization
			ET_c=ET_d*365
!			print *,'ET=',NPP_yrc,ET_c
            write(*,*)year,gpp_yr,NPP_yr,NEE_yr,N_fix_yr,N_up_yr
...
!     Plant growth and allocation, based on LM3V
     ...
     &              0.04*NSC,
     &              0.04*NSN*CNp0)
!     print *,'',GPmax*acP*St*W*fnsc*(1.-exp(-NSN),0.04*NSC,0.04*NSN*CNp0
...
!     carbon transfer according to Xu et al. 2007 
     ...
     &	           SNslow,SNpass,LAI,LAIMAX,kappaVcmax,N_immob,N_loss) 
      ...
      real L_fall,L_add,LAI,LAIMAX,SLA,rdepth,Sps
...
!     for nitrogen sub-model
      ...
      real Scalar_N_flow,Scalar_N_T,Scalar_N_uptake
...
!
      ...
      ksye=0.1      ! C cost per N for uptake
...
!Xia: close
	 ...
	 Scalar_N_uptake=exp(308.56*(1/56.02-1/(Tsoil+46.02)))	
...
!      OutC(1)=QC(1)/tauC(1)*S_omega
      ...
      OutC(4)=QC(4)/tauC(4)*S_omega* S_T(1)*SNfine!*CN(4)/CN0(4)
      OutC(5)=QC(5)/tauC(5)*S_omega* S_T(2)*SNcoarse!*CN(5)/CN0(5)
...
!	 N_fixation=0.1*max((0.0234*ET_c+0.172)/(10*365),0.)
!	 N_miner=N_miner+N_fixation

!	 QNminer=QNminer+N_miner+N_fixation
...
!      N_demand=NPP_L/CN0(1)+NPP_W/CN0(2)+NPP_R/CN0(3)+N_deficit
	 N_demand=(1/CN0(1)+1/CN0(2)+1/CN0(3))*(1+D_leaf)/CN(1)*LAI/LAIMAX
     &         *Scalar_N_uptake*NPP
	
      if(NSN<24.*30.*N_demand)then
		D_leaf=0.25*(1.-exp(-(2.*QN(1)/(2.852*LAIMAX))**(8)))    
       else
	   D_leaf=0.25*exp(-(2.*QN(1)/(2.852*LAIMAX))**(8))
      endif
		
!	 print *,'Dleaf',NPP,NSN,N_demand,Scalar_N_uptake
!     Nitrogen input:
	 
!     1. Nitrogen resorption
      N_transfer=(0.5*OutN(1)+0.2*OutN(3))*alphaN
      costCreuse= Creuse0*N_transfer
      N_demand=N_demand-N_transfer+N_immob
	  CN_plant=(QN(1)+QN(3)+NSN)/(QC(1)+QC(3)+NSC)
	  f_CN=max((1.7-CN_plant)/1.058,0.0)
...
!     2.  N uptake
     ...
     &		       0.514*QNminer*(0.05+1/(N_miner+0.83))
     &			   *Scalar_N_uptake*f_CN*0.01*QC(3),
     &               Nup0*NSC/(ksye/QNminer))
!     &              Nup0*NSC/(ksye/QNminer))
!		 print *,'N_uptake',QNminer,N_miner,Scalar_N_uptake,f_CN,QC(3)
 
	      else
			 N_uptake=0.0
	      endif
		 costCuptake=N_uptake*Amin1((ksye/QNminer),99.)
!     &		*Amin1(ksye/QNminer,Cfix0)		  
	 
        endif
      endif
		 N_demand=N_demand-N_uptake 
!	     elseif(N_demand>0.0)then
			N_fixation=Amin1((0.0234*ET_c+0.172)/(100*365),abs(N_demand))
...
!      print *,'alphaN=',N_leaf+N_wood+N_root
      N_LF=OutN(1)*(1.-0.2)
      N_WF=OutN(2)
      N_RF=OutN(3)*(1.-0.1)
...
!      Scalar_N_T=N_deN0*exp((Tsoil-25.)/10.)  Du: shift up 
...
!     update nitrogen pools
     ...
     &            + (OutN(1)*0.8 + etaW*OutN(2) + OutN(3)*0.9)
...
!     calculate N related scalars for Duke FACE
	 kappaVcmax=0.000009*0.11*CN(1)/(1-exp(-0.11*LAI))
!     &         *exp(-0.11*LAI)*(0.17+0.074*CN(1)/LAI)/1.152
      SNvcmax =exp(-kappaVcmax*(CN(1)-CN0(1))/CN0(1)) ! /CN0(1) ! Duke
	  SNvcmax=Amin1(exp(-CN_plant),1.)
      SNgrowth=Amin1(abs((N_demand-N_deficit)/(N_demand+N_deficit)),1.)  !exp(-(CN(1)-CN0(1))/CN0(1)) AMAX1((CNmax-CN_foliage)/(CNmax-CNmin),0.0)+0.25
