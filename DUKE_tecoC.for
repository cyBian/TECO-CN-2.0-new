...
!	     data CN0 /80.,354.,80.,40.,300.,10.,15.,8./ !DUKE
   		 data CN0 /31.,354.,67.,40.,300.,10.,15.,8./ !ORNL
...
! for diagnosing
 ...
 !Xia:close
      ...
      read(10,11)commts
      open(63,file="annual_duke_tecoC.csv")
...
!    	  QC=(/153.,1327.,248.,582.,472.,199.,530.,1221./) !initial data
!          QC=(/100.,800.,100.,39.,100.,122.,834.,12./)
!          QC=(/153.,5350.,337.,896.,1715.,400.,1353.,5233./)! for DUKE
...
!                      co2ca=(input_data(5,m)*ScCO2)*1.0E-6
                      co2ca=(input_data(5,m))*1.0E-6  !180
                  endif
...
!                 update vcmx0 and eJmx0 according to C/N of leaves
                  Vcmx0 = Vcmax0!*SNvcmax  !Du:2016/4/20
...
!                 THE Fourth PART: simulating C influx allocation in pools
     ...
     &               SNvcmax,SNgrowth,SNRauto,SNrs,S_omega,S_T)

!                 update NSC
                   Rauto  =Rmain+Rgrowth !+Rnitrogen   du:2016/4/9
...
!                 Rhetero=Rh_f + Rh_c + Rh_Micr + Rh_Slow + Rh_Pass
     ...
     &                    +Rh_pools(4)+Rh_pools(5)
                  Rsoil  =Rhetero+RmRoot+RgRoot !+Rnitrogen  du:2016/4/9
...
!Du added CN,QN for test  
	         ...
	         write(66,162)year,doy,alphaL_d,alphaW_d,alphaR_d!,omega_out
...
!     Plant growth and allocation, based on LM3V
     ...
     &              0.04*NSC)    
!     &              0.04*NSN*CNp0)   !Du:2016/4/9
!      c1=(bmR+200.)/bmL*CN(1)/CN0(1) !+N_deficit/NSN
      c1=bmL/bmR  !*CN(1)/CN0(1)+N_deficit/NSN    !Du:2016/4/9
!      c2 = 0
...
!	  print *,'L',gamma_W+gamma_T+gamma_N,bmleaf
...
!     N scalars on decomposition
      SNfine  =1. !exp(-(CN0(4)-CN(4))/CN0(4)) 
      SNcoarse=1. !exp(-(CN0(5)-CN(5))/CN0(5)) 
      SNmicr  =3. !exp(-(CN0(6)-CN(6))/CN0(6)) 
      SNslow  =1. !exp(-(CN0(7)-CNC(7))/CN0(7)) 
      SNpass  =1. !exp(-(CN0(8)-CN(8))/CN0(8)) 
...
!      OutC(1)=QC(1)/tauC(1)*S_omega
      ...
      OutC(4)=QC(4)/tauC(4)*S_omega* S_T(1)!*CN(4)/CN0(4)   du:2016/4/9      !*SNfine
      OutC(5)=QC(5)/tauC(5)*S_omega* S_T(2)!*CN(5)/CN0(5)   du:2016/4/9       !*SNcoarse
...
!     2.  N uptake
     ...
     &                       Nup0*NSC/(ksye/QNminer))
          ...
          else!if(NSN<24.*30.*N_demand)then
!     3.  Nitrogen fixation
...
!     calculate N related scalars for Duke FACE
      kappaVcmax=1.!CN0(1)/1.
      SNvcmax =1.!exp(-kappaVcmax*(CN(1)-CN0(1))/CN0(1)) ! /CN0(1) ! Duke
      SNgrowth=1.!exp(-(CN(1)-CN0(1))/CN0(1)) !  AMAX1((CNmax-CN_foliage)/(CNmax-CNmin),0.0)+0.25
      SNRauto =1.!exp(-(CN(1)-CN0(1))/CN0(1)) !  AMAX1((CNmax-CN_foliage)/(CNmax-CNmin),0.0)+0.5
      SNrs=1.
