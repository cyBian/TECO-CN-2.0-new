...
!      CN_leaf,CN_wood,CN_root,CN_fine,CN_coarse,CN_micr,CN_slowC,CN_pass
      ...
      real N_up_d,N_fix_d,N_dep_d,N_leach_d,N_vol_d,N_immob_d,N_miner_d
      real N_up_yr,N_fix_yr,N_dep_yr,N_leach_yr,N_vol_yr,N_immob_yr
      real N_miner,alphaN,N_miner_yr
      real SNvcmax,SNgrowth,SNRauto,SNrs
	  real SNfine,SNcoarse,SNmicr,SNslow,SNpass,SNwood,SNleaf
	  real SNroot
...
!      for phenology
      ...
      real Ra_Nfix,Rh_Nfix,N_limit
      real gpp_yr,NPP_yr,NEE_yr,RaL_yr,RaR_yr,RaS_yr,Rh_yr,NPP_yrc,ET_c
      real R_Ntr_yr,Ndemand_yr	  !Du:for retranslocated
	  real N_demand_d,Nmin_yr
	  common NPP_yrc,ET_c,N_limit,Ndemand_yr  !Du:for mineralization
...
!      data CN0 /35.,500.,42.,40.,300.,10.,15.,8./
!     Default C/N ratios of Duke FACE
      data CN0 /40.,354.,67.,50.,300.,10.,15.,8./
...
!      open(101,file="FACE_Daily_Output")
      open(102,file="H2_FACE_Traceability_Output.csv")
! for diagnosing
       open(105,file='H2_Diagnosing.csv')
 !Xia:close
      open(10,file=parafile,status='old',ACTION='read',IOSTAT=istat1)
      read(10,11)commts
      open(63,file="H2_annual_duke_tecocnH2.csv")
      open(64,file="H2_daily_duke.csv")
!	  open(65,file="annual_spinup.csv")
	  open(66,file="H2_daily_spinup.csv")
...
!	     write (66,'(a250)') "alphaL_d,alphaW_d,alphaR_d,omega_out"
...
!         QC=(/320.,4021.,433.,89.,461.,38.,1629.,235./)

!	  QC=(/153.,3887.,480.,987.,1304.,354.,1125.,2117./) !initial data for duke
	  QC=(/153.,5350.,337.,896.,1715.,400.,1353.,5233./)
!        QC=(/153.,3887.,372.,987.,1304.,354.,1125.,2117./)
!         QC=(/100.,4300.,500.,100.,694.,1000.,1585.,4000./) !based on Xu et al.2007
!		QC=(/800.,4300.,1000.,100.,694.,1000.,1585.,4000./)  !for ORNL
...
!                 update vcmx0 and eJmx0 according to C/N of leaves
                  Vcmx0 = Vcmax0*SNvcmax*N_limit
!                  eJmx0 = 2.7*Vcmx0  ! original
!                  eJmx0 = 1.67*Vcmx0 ! Weng 02/21/2011 Medlyn et al. 2002
                   eJmx0 = 1.9167*Vcmx0  !specific in O-CN Du 1/2/2018  Friend and Kiang,2005
...
!                 THE Fourth PART: simulating C influx allocation in pools
     ...
     &                S_T,SNleaf,SNwood,SNroot,!SNmicr,SNslow,SNfine,SNcoarse,
...
!                 update
				  ...
				  N_demand_d=N_demand_d+N_leaf+N_wood+N_root !Du: for retranslocated
                  N_LF_d=N_LF_d+N_LF
                  N_WF_d=N_WF_d+N_WF
                  N_RF_d=N_RF_d+N_RF
				  ...
				  N_miner_yr=N_miner_yr+N_miner
111				  Ndemand_yr=Ndemand_yr+N_leaf+N_wood+N_root  !Du:for retranslocated
...
162           format(I6,",",f8.0,",",120(f12.4,","))
            ...
            stor_use=Storage/times_storage_use
			NPP_yrc=NPP_yr!Du:for calculate BNF
			ET_c=ET_d*356
            write(*,*)year,gpp_yr,NPP_yr,NEE_yr,N_fix_yr,N_fix_d
            write(63,161)yr,year,gpp_yr,R_Ntr_yr,NPP_yr,Rh_yr,NEE_yr,
     &                   rain_yr,transp_yr,evap_yr,runoff_yr,           
     &                   N_up_yr,N_fix_yr,N_dep_yr,N_leach_yr,N_vol_yr,
     &                   N_deficit,(QC(i),i=1,8),(QN(i),i=1,8),
     &                 QNminer,N_miner_yr,N_immob_yr
...
!     Plant growth and allocation, based on LM3V
      GPmax=SNgrowth*(GLmax*bmL+GSmax*StemSap+GRmax*bmR)/acP
      GrowthP=AMIN1(GPmax*acP*St*W*fnsc*(1.-exp(-NSN)),
     &              0.04*NSC,
     &              0.04*NSN*CNp0)
!      c1=(bmR+200.)/bmL*CN(1)/CN0(1) !+N_deficit/NSN
      c1=bmL/bmR*CN(1)/CN0(1)+abs(N_deficit/NSN)
!      c2 = 0
...
!     carbon transfer according to Xu et al. 2007 
     ...
     &               SNpass,N_immob,N_loss)
      ...
      implicit none
      real NPP,NPP_L,NPP_W,NPP_R,NPP_yrc,ET_c,N_limit
	  real Ndemand_yr
	  common NPP_yrc,ET_c,N_limit,Ndemand_yr
...
!     for nitrogen sub-model
      ...
      real SNvcmax,SNgrowth,SNRauto,SNrs
      real kappaVcmax
      real SNfine,SNcoarse,SNmicr,SNslow,SNpass,SNwood
	  real SNroot,SNleaf
...
!
      ...
      Cfix0=2.      ! C cost per N for fixation
      ksye=0.02      ! C cost per N for uptake
...
!	SNfine= OutC(4)*(1. - f_F2M-(CN(6)/CN(4)))/CN(6)
!	SNcoarse=OutC(5)*(1. - f_C2M - f_C2S-(CN(7)/CN(5)))/CN(7)
!	SNmicr=OutC(6)* (1. - f_M2S - f_M2P-(CN(7)/CN(6)))/CN(7) !negativ
!	SNslow=OutC(7)* (1. - f_S2P - f_S2M-(CN(8)/CN(7)))/CN(8)
!	SNpass=OutC(8)/CN(8)
!	print *,'SNf=',SNpass
!========================================================================
!     Nitrogen part
...
!	N_miner=SNfine+SNcoarse+SNslow+SNpass
	 ...
	 N_fixation=2*1.8*(1-exp(-0.003*NPP_yrc))/86400  !Du:BNF-npp
!	 N_fixation=(0.0102*ET_c+0.524)/(10*86400*365)
	 N_miner=N_miner+N_fixation
...
!     1. Nitrogen resorption
	 N_transferL=L_fall*(CN(4)-CN(1))/(CN(1)*CN(4))
	 N_transfer=AMin1(N_transferL,
     &           365*86400*N_transferL*N_demand/Ndemand_yr)
	 
	  alphaN=(CN(4)-CN(1))/(CN(1)*CN(4))*L_fall/QC(1)
!      N_transfer=(OutN(1) + OutN(2) +OutN(3))*alphaN
      costCreuse= Creuse0*N_transfer
      N_demand=N_demand-N_transfer
!	 print *,'N_transfer=',N_fixation

      If(N_demand>0.0)then

!     2.  N uptake
          if(N_demand+N_immob>QNminer)then
!		          if(NSN.lt.NSNmax)then
              N_uptake=QNminer*QNminer/(N_demand+N_immob)
			  N_limit=QNminer/(N_demand+N_immob)
	              else
		          N_uptake=0.0
				  endif
			else
...
!     update NSN
      NSN=NSN+N_transfer+N_uptake
!      print *, 'N=',NSN
!     Total C cost for nitrogen
      Rnitrogen=costCuptake+costCreuse+costCfix
...
!     Nitrogen using, non-structural nitrogen pool, NSN
      N_leaf =AMIN1(NPP*alpha_L/CN(1),0.2*NSN)
      N_wood =AMIN1(NPP*alpha_W/CN(2)                         ,0.1*NSN)
      N_root =AMIN1(NPP*alpha_R/CN(3),0.2*NSN)
...
!     calculate N related scalars for Duke FACE
      kappaVcmax=N_limit!CN0(1)/1.
      SNvcmax =exp(-kappaVcmax*(CN(1)-CN0(1))/CN0(1)) ! /CN0(1) ! Duke
      SNgrowth=N_limit*exp(-(CN(1)-CN0(1))/CN0(1)) !  AMAX1((CNmax-CN_foliage)/(CNmax-CNmin),0.0)+0.25
      SNRauto =N_limit*exp(-(CN(1)-CN0(1))/CN0(1)) !  AMAX1((CNmax-CN_foliage)/(CNmax-CNmin),0.0)+0.5
      SNrs=1.
      return
      end
